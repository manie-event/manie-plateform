...
function computedAsync(evaluationCallback, initialState, optionsOrRef) {
  ...
  const started = shallowRef2(!lazy);
...
function useAsyncQueue(tasks, options) {
  ...
  const activeIndex = shallowRef2(-1);
...
function useAsyncState(promise, initialState, options) {
  ...
  async function execute(delay2 = 0, ...args) {
    if (resetOnExecute)
      state.value = toValue2(initialState);
    error.value = void 0;
    isReady.value = false;
...
function useBase64(target, options) {
  const base64 = shallowRef2("");
  const promise = shallowRef2();
  function execute() {
    if (!isClient)
      return;
...
function useBluetooth(options) {
  ...
  const server = shallowRef2();
  const isConnected = shallowRef2(false);
  function reset() {
  ...
  return {
    isSupported,
    isConnected: readonly2(isConnected),
    // Device:
    device,
    requestDevice,
    // Server:
...
function useBreakpoints(breakpoints, options = {}) {
  ...
  const mounted = ssrSupport ? shallowRef2(false) : { value: true };
  if (ssrSupport) {
...
function useBroadcastChannel(options) {
  ...
  const isClosed = shallowRef2(false);
  const channel = ref2();
  const data = ref2();
  const error = shallowRef2(null);
  const post = (data2) => {
...
function useBrowserLocation(options = {}) {
  ...
  for (const [key, ref3] of objectEntries(refs)) {
    watch2(ref3, (value) => {
...
function useClipboardItems(options = {}) {
  ...
  return {
    isSupported,
    content: shallowReadonly2(content),
    copied: readonly2(copied),
    copy,
    read: updateContent
  };
}
...
function useCloned(source, options = {}) {
  const cloned = ref2({});
  const isModified = shallowRef2(false);
  let _lastSync = false;
  const {
    manual,
    clone = cloneFnJSON,
...
function useStorage(key, defaults2, storage, options = {}) {
  ...
  const keyComputed = computed2(() => toValue2(key));
  if (!storage) {
    try {
...
function useConfirmDialog(revealed = shallowRef2(false)) {
...
function useCountdown(initialCountdown, options) {
  var _a, _b;
  const remaining = shallowRef2(toValue2(initialCountdown));
  ...
  const reset = (countdown) => {
    var _a2;
    remaining.value = (_a2 = toValue2(countdown)) != null ? _a2 : toValue2(initialCountdown);
  };
  const stop = () => {
    intervalController.pause();
    reset();
...
function useCssVar(prop, target, options = {}) {
  ...
  const variable = shallowRef2(initialValue);
...
function useCycleList(list, options) {
  ...
  watch2(listRef, () => set2(index.value));
  return {
    state,
    index,
    next,
...
function useDisplayMedia(options = {}) {
  var _a;
  const enabled = shallowRef2((_a = options.enabled) != null ? _a : false);
...
function useDraggable(target, options = {}) {
  ...
  const start = (e) => {
    var _a2;
    if (!toValue2(buttons).includes(e.button))
      return;
    if (toValue2(options.disabled) || !filterEvent(e))
      return;
    if (toValue2(exact) && e.target !== toValue2(target))
      return;
    const container = toValue2(containerElement);
    const containerRect = (_a2 = container == null ? void 0 : container.getBoundingClientRect) == null ? void 0 : _a2.call(container);
    const targetRect = toValue2(target).getBoundingClientRect();
    const pos = {
  ...
  if (isClient) {
    const config = () => {
      ...
      return {
        ...
        passive: !toValue2(preventDefault2)
      };
    };
...
function useDropZone(target, options = {}) {
  var _a, _b;
  const isOverDropZone = shallowRef2(false);
...
function useElementByPoint(options) {
  ...
  const element = shallowRef2(null);
  const cb = () => {
    var _a, _b;
...
function useEventSource(url, events2 = [], options = {}) {
  const event = shallowRef2(null);
  const data = shallowRef2(null);
  const status = shallowRef2("CONNECTING");
...
function useFetch(url, ...args) {
  ...
  const isFinished = shallowRef2(false);
  const isFetching = shallowRef2(false);
  const aborted = shallowRef2(false);
  const statusCode = shallowRef2(null);
...
function useFileDialog(options = {}) {
  ...
  const files = ref2(prepareInitialFiles(options.initialFiles));
  ...
  const applyOptions = (options2) => {
    ...
    if (!el)
      return;
    el.multiple = toValue2(options2.multiple);
    el.accept = toValue2(options2.accept);
    el.webkitdirectory = toValue2(options2.directory);
    if (hasOwn(options2, "capture"))
      el.capture = toValue2(options2.capture);
  };
  const open = (localOptions) => {
    ...
    if (toValue2(mergedOptions.reset))
      reset();
    el.click();
  };
  watchEffect2(() => {
...
function useFileSystemAccess(options = {}) {
  ...
  const fileHandle = shallowRef2();
  const data = shallowRef2();
  const file = shallowRef2();
  ...
  async function open(_options = {}) {
    if (!isSupported.value)
      return;
    const [handle] = await window2.showOpenFilePicker({ ...toValue2(options), ..._options });
  ...
  watch2(() => toValue2(dataType), updateData);
  return {
    isSupported,
    data,
    file,
...
function useFocus(target, options = {}) {
  ...
  const innerFocused = shallowRef2(false);
...
function useKeyModifier(modifier, options = {}) {
  ...
  const state = shallowRef2(initial);
  if (document2) {
      ...
      get(target2, prop, rec) {
        ...
        if (!(prop in refs)) {
          if (/[+_-]/.test(prop)) {
            ...
            refs[prop] = computed2(() => keys2.map((key) => toValue2(proxy[key])).every(Boolean));
          } else {
...
function useMediaControls(target, options = {}) {
  ...
  const listenerOptions = { passive: true };
  const currentTime = shallowRef2(0);
  const duration = shallowRef2(0);
  const seeking = shallowRef2(false);
  const volume = shallowRef2(1);
  const waiting = shallowRef2(false);
  const ended = shallowRef2(false);
  const playing = shallowRef2(false);
  const rate = shallowRef2(1);
  const stalled = shallowRef2(false);
  const buffered = ref2([]);
  const tracks = ref2([]);
  const selectedTrack = shallowRef2(-1);
  const isPictureInPicture = shallowRef2(false);
  const muted = shallowRef2(false);
  ...
  useEventListener(
    target,
    "timeupdate",
    () => ignoreCurrentTimeUpdates(() => currentTime.value = toValue2(target).currentTime),
    listenerOptions
  );
  useEventListener(
    target,
    "durationchange",
    () => duration.value = toValue2(target).duration,
    listenerOptions
  );
  useEventListener(
    target,
    "progress",
    () => buffered.value = timeRangeToArray(toValue2(target).buffered),
    listenerOptions
  );
  useEventListener(
    target,
  ...
  useEventListener(
    target,
    "ratechange",
    () => rate.value = toValue2(target).playbackRate,
    listenerOptions
  );
  useEventListener(
    target,
...
function useNavigatorLanguage(options = {}) {
  ...
  const language = shallowRef2(navigator2 == null ? void 0 : navigator2.language);
...
function useScreenOrientation(options = {}) {
  ...
  const angle = shallowRef2(screenOrientation.angle || 0);
  if (isSupported.value) {
...
function usePointer(options = {}) {
  ...
  const state = shallowRef2(options.initialValue || {});
...
function usePointerSwipe(target, options = {}) {
  ...
  const isSwiping = shallowRef2(false);
  const isPointerDown = shallowRef2(false);
...
function usePreferredLanguages(options = {}) {
  const { window: window2 = defaultWindow } = options;
  if (!window2)
    return shallowRef2(["en"]);
  const navigator2 = window2.navigator;
  const value = shallowRef2(navigator2.languages);
  useEventListener(window2, "languagechange", () => {
...
function useScreenSafeArea() {
  const top = shallowRef2("");
  const right = shallowRef2("");
  const bottom = shallowRef2("");
  const left = shallowRef2("");
  if (isClient) {
...
function useScriptTag(src, onLoaded = noop, options = {}) {
  ...
  const scriptTag = shallowRef2(null);
  let _promise = null;
...
function useScrollLock(element, initialState = false) {
  const isLocked = shallowRef2(initialState);
...
function useShare(shareOptions = {}, options = {}) {
  ...
  const share = async (overrideOptions = {}) => {
    if (isSupported.value) {
      const data = {
        ...toValue2(shareOptions),
        ...toValue2(overrideOptions)
      };
      let granted = true;
...
function useSpeechRecognition(options = {}) {
  ...
  const isListening = shallowRef2(false);
  const isFinal = shallowRef2(false);
...
function useSpeechSynthesis(text, options = {}) {
  ...
  const isPlaying = shallowRef2(false);
  const status = shallowRef2("init");
  ...
  const bindEventsForUtterance = (utterance2) => {
    ...
    utterance2.volume = toValue2(volume);
    utterance2.onstart = () => {
...
function useStorageAsync(key, initialValue, storage, options = {}) {
  ...
  const type = guessSerializerType(rawInit);
  const data = (shallow ? shallowRef2 : ref2)(toValue2(initialValue));
  const serializer = (_a = options.serializer) != null ? _a : StorageSerializers[type];
  if (!storage) {
    try {
...
function useStyleTag(css, options = {}) {
  const isLoaded = shallowRef2(false);
  const {
    document: document2 = defaultDocument,
    immediate = true,
    manual = false,
...
function useTextareaAutosize(options = {}) {
  ...
  const textareaScrollHeight = shallowRef2(1);
  const textareaOldWidth = shallowRef2(0);
  function triggerResize() {
    ...
    const _styleTarget = toValue2(options == null ? void 0 : options.styleTarget);
    if (_styleTarget)
...
function useTimeAgoIntl(time, options = {}) {
  ...
  const result = computed2(
    () => getTimeAgoIntlResult(new Date(toValue2(time)), options, toValue2(now2))
  );
  const parts = computed2(() => result.value.parts);
  const timeAgoIntl = computed2(
...
function useTransition(source, options = {}) {
  ...
  watch2(() => toValue2(options.disabled), (disabled) => {
    if (disabled) {
      currentId++;
...
function useUserMedia(options = {}) {
  var _a, _b;
  const enabled = shallowRef2((_a = options.enabled) != null ? _a : false);
  const autoSwitch = shallowRef2((_b = options.autoSwitch) != null ? _b : true);
...
function useWatchForSizes(size, list, containerRef, calculateRange) {
  watch2([size.width, size.height, () => toValue2(list), containerRef], () => {
    calculateRange();
  });
}
...
function useWakeLock(options = {}) {
  ...
  const requestedType = shallowRef2(false);
...
function useWebNotification(options = {}) {
  ...
  const permissionGranted = shallowRef2(isSupported.value && "permission" in Notification && Notification.permission === "granted");
...
function useWebSocket(url, options = {}) {
  ...
  const status = shallowRef2("CLOSED");
  const wsRef = ref2();
  const urlRef = toRef(url);
  let heartbeatPause;
  let heartbeatResume;
  ...
  const _init = () => {
    ...
    ws.onmessage = (e) => {
      if (options.heartbeat) {
        ...
        if (e.data === toValue2(responseMessage))
          return;
      }
      data.value = e.data;
...
function useWebWorkerFn(fn, options = {}) {
  ...
  const worker = ref2();
  const workerStatus = shallowRef2("PENDING");
  const promise = ref2({});
  const timeoutId = shallowRef2();
