...
<template>
  <v-card variant="flat" class="rounded-md card-hover overflow-hidden bg-surface">
    ...
    <div class="d-flex justify-end mr-3 mt-n6 position-absolute action-btn">
      <v-btn icon v-bind="props" color="white" size="small" class="mr-3"
    ...
    <v-card-item class="pt-3 text-left">
      ...
      <div class="d-flex align-center justify-start mt-2">
        <v-icon icon="mdi-star" color="warning" size="18" class="mr-1"></v-icon
        ><span class="font-weight-semibold text-grey200 text-subtitle-1">{{
          rating?.toFixed(1)
        }}</span>
      ...
      <div class="d-flex align-center justify-start gap-2 mt-2">
        ...
        <p class="text-h6 text-decoration-line-through font-weight-semibold text-grey100">
          ${{ offerPrice }}
        </p>
      </div>
    </v-card-item>
