                ...
                <v-row class="mt-2" dense>
                  ...
                  <template v-if="getVisibleField(section)">
                    <v-col cols="12">
                      <template
                        v-if="
                          getVisibleField(section)?.type === 'checkbox' &&
                          !getVisibleField(section)?.multiple
                        "
                      >
                        <v-switch
                          :label="getVisibleField(section)?.label"
                          :model-value="getSectionControllerValue(section)"
                          @update:model-value="(v) => setSectionControllerValue(section, !!v)"
                          density="comfortable"
                          :error="!!fieldErrors[getVisibleField(section)?.id || '']"
                          :error-messages="
                            fieldErrors[getVisibleField(section)?.id || '']
                              ? [fieldErrors[getVisibleField(section)?.id || '']]
                              : []
                          "
                        />
                    ...
                    <template v-if="!field.visibleSection && isFieldVisible(field, section)">
                      <v-col cols="12" md="6">
                        ...
                        <template v-else-if="field.type === 'checkbox' && field.multiple">
                          ...
                          <div v-if="fieldErrors[field.id]" class="text-error text-caption">
...
const includesAny = (source: any[] | undefined, values: any[] | undefined): boolean => {
...
const isMeaningfulSelection = (field: FieldSchema, val: any) => {
...
const onSectionControllerChanged = async (
...
const onMultiCheckboxChange = (field: FieldSchema, nextValue: any[]) => {
...
const gatherSelectedTokens = (section: SectionSchema): Set<string> => {
...
const getSectionControllerValue = (section: SectionSchema): boolean => {
  const sf = getVisibleField(section);
  return !!(sf && formState[sf.id]);
};

const setSectionControllerValue = (section: SectionSchema, v: boolean): void => {
  const sf = getVisibleField(section);
  console.log(sf, 'JE SAIS PAS CE QUE C');
...
const buildSelectionsFromSectorData = (): ServiceSelection[] => {
  ...
  for (const section of props.sections || []) {
    ...
    (sectorData.services || []).forEach((s) =>
      serviceNameToUuid.set(normalizeText(String(s.name)), String(s.uuid))
    );

    const keywordNameToUuid = new Map<string, string>();
    (sectorData.keywords || []).forEach((k) =>
      keywordNameToUuid.set(normalizeText(String((k as any).name)), String((k as any).uuid))
    );
...
const humanizeEventName = (value?: string, fallback = 'Événement'): string => {
  if (!value) return fallback;
  return value.replace(/_/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase());
};

const computeDateRange = (answers: Record<string, any>): [string, string] => {
...
/***** Navigation *****/
const isValueEmpty = (field: FieldSchema, value: any): boolean => {
